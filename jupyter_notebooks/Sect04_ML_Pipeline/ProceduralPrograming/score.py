...
def predict(data):
    ...
    data[config.YEAR_VARIABLE] = pf.elapsed_years(
        data, config.YEAR_VARIABLE, ref_var='YrSold'
    )
    ...
    # scale variables
    data = pf.scale_features(
        data[config.FEATURES], config.OUTPUT_SCALER_PATH
    )
